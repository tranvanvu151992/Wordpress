...
if( get_field('file_types', 'product_cat_'.$product_cat_id[0]) ): 
    $i = 1;
    while( has_sub_field('file_types', 'product_cat_'.$product_cat_id[0]) ):
        $select = get_sub_field_object('file_type');
        $file_type_option = array();
        foreach( $select['choices'] as $k => $v ):
            $file_type_option[$k] = $v;
        endforeach;
        if ($i == 1){ break; }
    endwhile;
endif;
...
//                $('#dokan_save_single_variation').click(function(){
//                    alert('123');
//                    return false;
//                })
//            })
                            ...
                            <div class="woocommerce_variation wc-metabox closed">
                                <h3 class="ui-sortable-handle">
                                    <# if( !_.isUndefined( data.variation_id ) ) { #>
                                        <strong> #{{ data.variation_id[0] }}</strong>
                                    <# } else { #>
                                    ...
                                    <# _.each( data.variation_attributes[0], function( title, index ) { #>
                                        ...
                                        <select name="attribute_{{index}}[]" style="display: none;">
                                ...
                                <table cellspacing="0" cellpadding="0" class="woocommerce_variable_attributes wc-metabox-content">
                                    <tbody>
                                        <tr>
                                            <td style="display:none" colspan="2" class="sku">
                                                ...
                                                <input placeholder="Please add Sku" type="text" placeholder="" value="SKU-{{ data.variation_id[0] }}" name="variable_sku[]" size="7">
                                            </td>
                                            <td rowspan="2" class="data">
                                                <table cellspacing="0" cellpadding="0" class="data_table">
                                                    <tbody>
                                                                                ...
                                                                                <th colspan="4">
                                                                                    <a href="#" class="insert-file-row btn btn-sm btn-success" data-row='
                                                                                        <tr>
                                                                                            ...
                                                                                            <td class="file_url gin"><input type="text" class="input_text wc_file_url" placeholder="<?php _e( "http://", "dokan" ); ?>" name="_wc_variation_file_urls[{{ ( !_.isUndefined( data.variation_id ) ) ? data.variation_id[0] : ''  }}][]" value="" /></td>
                                                        ...
                                                        <tr style="display:none;" class="show_if_variation_downloadable hidden-important">
                                                            <td>
                                                                <div>
                                        ...
                                        <tr class="hidden-important">
                                            ...
                                            <td class="options">
                                                <label class="checkbox" style="display: none"><input type="checkbox"  {{ ( !_.isUndefined( data.variation_post_status ) && ( data.variation_post_status[0] == 'publish' ) ) ? 'checked="checked"': '' }} name="variable_enabled[]" class="checkbox"> Enabled</label>

                                                <label class="checkbox" style="display: none"><input type="checkbox" checked="checked" name="variable_is_downloadable[]" class="checkbox variable_is_downloadable"> Downloadable <a href="#" title="" class="tips" data-original-title="Enable this option if access is given to a downloadable file upon purchase of a product">[?]</a></label>

                                                <label class="checkbox" style="display: none"><input type="checkbox" {{ ( !_.isUndefined( data._virtual ) && data._virtual == 'yes' ) ? 'checked="checked"': '' }} name="variable_is_virtual[]" class="checkbox variable_is_virtual"> Virtual <a href="#" title="" class="tips" data-original-title="Enable this option if a product is not shipped or there is no shipping cost">[?]</a></label>

                                                <label class="checkbox" style="display: none"><input type="checkbox" {{ ( !_.isUndefined( data._manage_stock ) && data._manage_stock == 'yes' ) ? 'checked="checked"': '' }} name="variable_manage_stock[]" class="checkbox variable_manage_stock"> Manage Stock? <a href="#" title="" class="tips" data-original-title="Enable this option to enable stock management at variation level">[?]</a></label>
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
