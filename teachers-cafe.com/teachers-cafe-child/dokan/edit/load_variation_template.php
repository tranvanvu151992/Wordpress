...
?>
<p id="guide-upload" style="background: #808080 none repeat scroll 0% 0%; color: white; font-size: 17px; padding: 5px 10px;">Please select File type > Select Price > Upload File > Hit "Upload"</p>
<table class="dokan-table dokan-variations-table">
    <thead>
        <tr>
            <th></th>
            <!-- <th><?php _e( 'Image', 'dokan' ) ?></th> -->
            <th><?php _e( 'File Type', 'dokan' ) ?></th>
    ...
    <?php
        ...
        foreach ( $variations as $variation ) {
            ...
            <?php
                ...
                if (empty($total_variable_size)){ $total_variable_size = 0;}

                echo '<input type="hidden" class="link_size" name="link_size" id="link_size_'.$loop.'" value="'.$total_variable_size.'" />';
                echo '<input type="hidden" class="has_link_file" name="has_file" id="has_link_size_'.$loop.'" value="no">';
                echo '<input type="hidden" class="variation_data_id" name="variation_data_id" id="variation_data_item_prize_'.$loop.'" value="'.esc_attr( $variation_id ).'" />';
...
<div class="dokan-variation-action-wrapper">
    ...
    <a href="#variation-edit-popup" data-post_id='<?php echo $post_id ?>' class="dokan-btn dokan_add_new_variation dokan-btn-theme dokan-right"><?php _e( 'Add File Type', 'dokan' ); ?></a>
