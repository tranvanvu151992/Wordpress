...
<?php if ( ! post_password_required( $current_page_id ) ) : ?>
	...
	<?php if ( is_array( $categories_to_display_slugs_names ) && ! empty( $categories_to_display_slugs_names ) && 'no' != fusion_get_page_option( 'portfolio_filters', $current_page_id ) ) : ?>
		...
		<script type="text/javascript">
			jQuery(document).ready(function(){
				jQuery('.filters-select').on('change', function() {
					...
					jQuery('.fusion-filters_hidden .'+value).click();
				})
			});
		</script>
		<div style="display: none !important;">
		<ul style="" class="fusion-filters fusion-filters_hidden clearfix">
			<li class="fusion-filter fusion-filter-all fusion-active">
				<a class="allskillssss" data-filter="*" href="#"><?php echo apply_filters( 'avada_portfolio_all_filter_name', esc_html__( 'Discipline', 'Avada' ) ); ?></a>
			</li>
			<?php
			...
			foreach ( $terms as $term ) {
				echo '<li class="fusion-filters"><a class="'.$term->slug.'" href="#" data-filter=".'.$term->slug.'">'.$term->name.'</a></li>';
			}
			?>
		</ul>
		</div>
		...
		<select class="filters-select">
			...
			echo '<option value="allskillssss">Discipline</option>';
			foreach ( $terms as $term ) {
	...
	<div class="fusion-portfolio-wrapper" data-picturesize="<?php echo $data_picture_size; ?>" data-pages="<?php echo $portfolio_posts_to_display->max_num_pages; ?>">
		...
		<?php while ( $portfolio_posts_to_display->have_posts() ) : $portfolio_posts_to_display->the_post(); ?>
			...
			<!-- the portfolio post -->
			<?php 
				$termsArray = get_the_terms( $post->ID, "portfolio_skills" );  //Get the terms for this particular item
				$termsString = ""; 
				$name = "";
		...
		?>
		<div class="fusion-portfolio-post post-<?php echo get_the_ID(); ?> <?php echo $post_classes; ?> <?php echo $termsString; ?> ">
			<div class="ct-bottomct">
				<h3 class="skilltitle"><?php the_title(); ?></h3>
				<span class="skillsss"><?php echo $name; ?></span>
			</div>
			<?php
				/**
